...
void pbstream_trajectories_to_bag(const std::string& pbstream_filename,
  ...
  cartographer::io::ProtoStreamReader reader(FLAGS_input);
  cartographer::io::ProtoStreamDeserializer deserializer(&reader);

  cartographer::mapping::proto::PoseGraph pose_graph = deserializer.pose_graph();

  rosbag::Bag bag(output_bag_filename, rosbag::bagmode::Write);
  for (const auto trajectory : pose_graph.trajectory()) {
    const auto child_frame_id = "trajectory_" + std::to_string(trajectory.trajectory_id());
    LOG(INFO)
